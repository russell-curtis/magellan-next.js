// scripts/seed-st-kitts-workflow.ts
...
async function seedStKittsWorkflow() {
  try {
    console.log('🌱 Seeding St. Kitts workflow...')
    ...
    if (!stKittsProgram) {
      console.error('❌ St. Kitts program not found. Please run seed-crbi-programs.ts first.')
      process.exit(1)
    }
    ...
    if (existingTemplate.length > 0) {
      console.log('✅ St. Kitts workflow template already exists')
      workflowTemplate = existingTemplate[0]
    } else {
      ...
      console.log('✅ Created St. Kitts workflow template:', workflowTemplate.id)
    }
    ...
    if (existingStages.length === 0) {
      ...
      const documentRequirementsData = [
        ...
        // Stage 3: Due Diligence (handled by government)
        {
          programId: stKittsProgram.id,
        ...
        // Stage 5: Government Submission (handled by CIU)
        {
          programId: stKittsProgram.id,
      ...
      console.log(`✅ St. Kitts workflow stages already exist (${existingStages.length} stages)`)
    }
    
    console.log('🎉 St. Kitts workflow seeded successfully!')
